...
function AppContent() {
  ...
  // Also check port for admin panel (port 5001 = admin, 5000 = customer)
  useEffect(() => {
    const port = window.location.port;
    
    // On port 5001, after login redirect to admin panel, not customer panel
    if (currentUser && (role === "master" || role === "admin" || role === "agent")) {
      if (port === "5001") {
        // Admin port - redirect to appropriate admin panel
        const adminPanelPath = role === "master" ? "/ms-panel-9921" : 
                               role === "admin" ? "/ad-panel-4432" : "/ag-panel-7781";
        if (!location.startsWith(adminPanelPath)) {
          setLocation(adminPanelPath + "/");
        }
        return;
      }
    }
