...
export default function CustomerDashboardNew() {
  ...
  return (
    <div className="p-2 sm:p-4 space-y-4 sm:space-y-6">
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-2 sm:gap-3 min-w-0">
          ...
          <div className="min-w-0">
            <h2 className="font-semibold text-base sm:text-lg truncate" data-testid="text-user-phone">{myAccount?.phone || myAccount?.email || "User"}</h2>
            <p className="text-xs sm:text-sm text-muted-foreground truncate" data-testid="text-user-email">{myAccount?.email}</p>
          </div>
        </div>
      ...
      <div className="bg-gradient-to-r from-blue-500 to-blue-600 rounded-2xl p-3 sm:p-5 text-white shadow-lg">
        <div className="flex items-center justify-between mb-2">
          <span className="text-blue-100 text-xs sm:text-sm">Informasi Saldo</span>
          <Badge variant="secondary" className="bg-white/20 text-white border-0 text-xs">
            <Sparkles className="h-3 w-3 mr-1" />
            <span className="hidden sm:inline">Kredit skor </span>{myAccount?.creditScore ?? 100}
          </Badge>
        </div>
        <div className="flex items-center gap-2 mb-3">
          <span className="text-2xl sm:text-3xl font-bold" data-testid="text-balance">
      ...
      <Card className="p-3 sm:p-5">
        <h3 className="font-semibold text-base sm:text-lg mb-1">Aksi Cepat</h3>
        <p className="text-xs sm:text-sm text-muted-foreground mb-3 sm:mb-4">Akses fitur utama dengan cepat</p>
        
        <div className="space-y-2 sm:space-y-3">
          {quickActions.map((action) => (
            <Link key={action.title} href={action.url}>
              <div className="flex items-center gap-3 sm:gap-4 p-2 sm:p-3 rounded-xl border hover-elevate cursor-pointer" data-testId={action.testId}>
                <div className={`h-10 w-10 sm:h-12 sm:w-12 rounded-xl ${action.color} flex items-center justify-center flex-shrink-0`}>
