...
export default function AuthPage() {
  ...
  useEffect(() => {
    const verifyCode = async () => {
      ...
      try {
        const response = await fetch(getApiUrl(`/api/verify-invitation/${encodeURIComponent(regInvitationCode)}`));
  ...
  return (
    <div className="flex items-center justify-center min-h-[80vh] px-2 sm:px-4">
      <Card className="w-full max-w-md">
        <CardHeader className="text-center space-y-0 p-4 sm:p-6">
        ...
        <CardContent className="px-4 sm:px-6">
          <Tabs defaultValue="login" className="w-full">
            <TabsList className="grid w-full grid-cols-2 mb-4 sm:mb-6">
              <TabsTrigger value="login" data-testid="tab-login" className="text-xs sm:text-sm">
        ...
        <CardFooter className="flex flex-col text-center space-y-2 p-4">
